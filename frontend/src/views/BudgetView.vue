<template>
  <div>
    <h2 class="text-2xl font-bold text-gray-900">預算設定</h2>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">月預算</th>
            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">本月已花</th>
          ...
          <tr v-for="item in budgetItems" :key="item.id" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-sm">
              <input
                ...
                min="0"
                step="100"
                class="w-24 px-2 py-1 border border-gray-300 rounded text-sm focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <td class="px-4 py-3 text-sm">
              <div v-if="item.budget > 0" class="flex items-center space-x-2">
                ...
                <span :class="item.over ? 'text-red-600 font-medium' : 'text-gray-600'" class="text-xs">
                  {{ item.percentage }}%
                </span>
              </div>
...
async function loadData() {
  try {
    ...
    const categories = catResult.categories.filter(c => c.id !== 11) // 排除收入
